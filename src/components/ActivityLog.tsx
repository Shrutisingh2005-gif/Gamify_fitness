...
import { Activity, Droplets, Moon, Utensils, Plus, Clock, Award, Zap, Calculator, Sparkles, CheckCircle2, Target } from 'lucide-react';
...
import { ActivityLog as ActivityLogType, UserProfile, UserGoal } from '../types';
import { estimateCalories, getDailySuggestions } from '../services/gemini';
import { motion } from 'motion/react';
import { calculateRank } from '../utils/rank';
import { GOAL_TARGETS } from '../constants/goals';
...
export default function ActivityLog({ userProfile }: { userProfile: UserProfile | null }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // Refresh suggestions when logs change
      const today = new Date().toDateString();
      const todayLogs = logs.filter(l => l.type === selectedType && new Date(l.timestamp?.toDate()).toDateString() === today);
      fetchSuggestions(todayLogs);
    }, (error) => {
      handleFirestoreError(error, OperationType.LIST, 'activities');
    });

    return () => unsubscribe();
  }, [selectedType, userProfile?.goal]);

  const fetchSuggestions = async (todayLogs: ActivityLogType[]) => {
    setLoadingSuggestions(true);
    const result = await getDailySuggestions(selectedType, todayLogs, userProfile?.goal);
  ...
  const handleSubmit = async (e?: React.FormEvent, customData?: any) => {
    ...
    try {
      ...
      // Streak logic
      const calculateNewStreak = (lastActiveStr: string, currentStreak: number) => {
      ...
      const newStreak = calculateNewStreak(userProfile?.lastActive || '', userProfile?.streak || 0);
        ...
        <div className="bg-white p-8 rounded-3xl border border-black/5 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            ...
            <button 
              onClick={() => {
                const today = new Date().toDateString();
                const todayLogs = history.filter(l => l.type === selectedType && new Date(l.timestamp?.toDate()).toDateString() === today);
                fetchSuggestions(todayLogs);
              }}
        ...
        {userProfile?.goal && (
          <div className="bg-white p-8 rounded-3xl border border-black/5 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-xl font-bold">Goal: {userProfile.goal}</h3>
            </div>
            
            <div className="space-y-6">
              {Object.entries(GOAL_TARGETS[userProfile.goal]).map(([key, target]) => {
                const todayTotal = history
                  .filter(l => l.type === key && new Date(l.timestamp?.toDate()).toDateString() === new Date().toDateString())
                  .reduce((acc, l) => acc + l.value, 0);
                const isEnough = todayTotal >= target;
                const type = activityTypes.find(t => t.id === key);
                
                return (
                  <div key={key} className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="font-bold capitalize">{key}</span>
                      <span className={isEnough ? 'text-emerald-500 font-bold' : 'text-black/40'}>
                        {todayTotal} / {target} {type?.unit}
                      </span>
                    </div>
                    <div className="w-full bg-black/5 rounded-full h-2 overflow-hidden">
                      <motion.div 
                        initial={{ width: 0 }}
                        animate={{ width: `${Math.min(100, (todayTotal / target) * 100)}%` }}
                        className={`h-full rounded-full ${isEnough ? 'bg-emerald-500' : 'bg-black/20'}`}
                      />
                    </div>
                    {isEnough ? (
                      ...
                      <p className="text-[10px] text-black/40 font-medium italic">
                        Keep going! You need {target - todayTotal} more {type?.unit}.
                      </p>
                    )}
                  </div>
                );
